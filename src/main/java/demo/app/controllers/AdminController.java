...
public class AdminController {
	...
	@GetMapping(path = { "/superapp/admin/users" }, produces = MediaType.APPLICATION_JSON_VALUE)
	public UserBoundary[] getAllUsers() {
		return adminLogic.getAllUsers().toArray(new UserBoundary[0]);
	}

	@GetMapping(path = { "/superapp/admin/miniapp" }, produces = MediaType.APPLICATION_JSON_VALUE)
	public MiniAppCommandBoundary[] getAllCommands() {
		return adminLogic.getAllCommands().toArray(new MiniAppCommandBoundary[0]);
	}

	@GetMapping(path = { "/superapp/admin/miniapp/{miniAppName}" }, produces = MediaType.APPLICATION_JSON_VALUE)
